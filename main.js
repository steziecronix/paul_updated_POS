  ...
  ipcMain.handle(CH.SETTINGS.RESET_DATA, (e, { scope }) => {
    ...
    const tx = db.transaction(() => {
      if (scope === 'today' || scope === 'month') {
        const dateExpr = { match: scope === 'today' ? new Date().toISOString().slice(0, 10) : new Date().toISOString().slice(0, 7) };
        const col = (colName) => scope === 'today' ? `date(${colName})` : `strftime('%Y-%m', ${colName})`;

        db.prepare(`DELETE FROM sale_items WHERE sale_id IN (SELECT id FROM sales WHERE ${col('date_time')} = ?)`).run(dateExpr.match);
        db.prepare(`DELETE FROM sales WHERE ${col('date_time')} = ?`).run(dateExpr.match);
        db.prepare(`DELETE FROM stock_purchase_items WHERE purchase_id IN (SELECT id FROM stock_purchases WHERE ${col('date')} = ?)`).run(dateExpr.match);
        db.prepare(`DELETE FROM stock_purchases WHERE ${col('date')} = ?`).run(dateExpr.match);
        db.prepare(`DELETE FROM stock_adjustments WHERE ${col('date')} = ?`).run(dateExpr.match);
        db.prepare(`DELETE FROM expenses WHERE ${col('date')} = ?`).run(dateExpr.match);
        db.prepare(`DELETE FROM shifts WHERE ${col('clock_in')} = ?`).run(dateExpr.match);
        db.prepare(`DELETE FROM customer_payments WHERE ${col('date')} = ?`).run(dateExpr.match);
      } else if (scope === 'full') {
